...
import { resolveFeedOwner } from '../../../lib/feed-owner';
...
export async function GET(request: Request) {
  try {
    ...
    const ownerKey = await resolveFeedOwner(request);
    const feeds = await listFeeds(ownerKey);
...
export async function POST(request: Request) {
  try {
    ...
    if (resolved.feed) {
      const id = await feedId(`${ownerKey}:${resolved.feed.kind}:${resolved.sourceUrl}:${resolved.feed.url}`);
      await saveFeed({
        id,
    ...
    const id = await feedId(`${ownerKey}:${resolved.sourceUrl}`);
    await saveFeed({
      id,
      source_url: resolved.sourceUrl,
      title,
...
export async function DELETE(request: Request) {
  try {
    ...
    const deleted = await deleteFeed(id, ownerKey);
